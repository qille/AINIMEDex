                ...
                <div key={index} className="pool-card">
                  ...
                  <div className="pool-actions">
                    <button
                      ...
                      onClick={() => {
                        console.log('Navigating to:', `/liquidity/${pair.pairAddress}`, 'Pair:', pair);
                        if (pair.status === 'Coming Soon' || pair.pairAddress === 'Not Created') {
                          showNotification('Pasangan ini belum tersedia.', 'info');
                          return;
                        }
                        if (!walletData.address || walletData.chainId.toLowerCase() !== '0x40d9') {
                          showNotification('Silakan hubungkan dompet Anda ke 0G Galileo Testnet.', 'error');
                          return;
                        }
                        if (!pair.pairAddress || !ethers.utils.isAddress(pair.pairAddress)) {
                          showNotification('Alamat pasangan tidak valid.', 'error');
                          return;
                        }
                      ...
                      disabled={isLoading || pair.status === 'Coming Soon' || pair.pairAddress === 'Not Created'}
                    >
                      Deposit
                    </button>
                  </div>
                    ...
                    <div className="pool-actions">
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (!position.pairAddress || !ethers.utils.isAddress(position.pairAddress)) {
                            showNotification('Alamat pasangan tidak valid.', 'error');
                            return;
                          }
