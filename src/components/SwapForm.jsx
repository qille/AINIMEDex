...
  // BARIS BARU: Menggunakan useRef untuk menyimpan objek Audio
  const flippyAudio = useRef(null);

  useEffect(() => {
    // BARIS BARU: Memuat audio sekali saat komponen dimuat
    flippyAudio.current = new Audio(flippySound);
    return () => {
      // BERSIHKAN: Jeda dan hapus objek audio saat komponen tidak digunakan
...
  // BARIS BARU: Fungsi untuk memutar suara
...
      const tokenA = tokenInAddress < tokenOutAddress ? tokenInAddress : tokenOutAddress;
      const tokenB = tokenInAddress < tokenOutAddress ? tokenOutAddress : tokenInAddress;
      const isReverse = tokenInAddress > tokenOutAddress;
...
        setAmountOut('0');
        setPairPrice('');
        toast.error('Token pair not found.', { position: 'bottom-left' });
        return;
      }
      const [price, reserveA, reserveB] = await routerContract.getTokenPrice(tokenA, tokenB, true);
...
      let amountOutWei;
      if (isReverse) {
        amountOutWei = amountInWei.mul(ethers.utils.parseUnits('1', 18)).div(price);
      } else {
        amountOutWei = amountInWei.mul(price).div(ethers.utils.parseUnits('1', 18));
      }
      const slippageFactor = slippage === 'auto' ? 1 : 1 - parseFloat(slippage) / 100;
      amountOutWei = amountOutWei.mul(Math.floor(slippageFactor * 100)).div(100);
      if (tokenOutAddress === '0x1a0326c89c000C18794dD012D5055d9D16900f77') {
        amountOutWei = amountOutWei.mul(99).div(100);
      }
      const amountOutFormatted = parseFloat(ethers.utils.formatUnits(amountOutWei, 18)).toFixed(4);
      setAmountOut(amountOutFormatted);
      const priceFormatted = isReverse
        ? parseFloat(ethers.utils.formatUnits(ethers.utils.parseUnits('1', 18).div(price), 18)).toFixed(4)
        : parseFloat(ethers.utils.formatUnits(price, 18)).toFixed(4);
...
    setAmountOut('0');
    setPairPrice('');
    // BARIS BARU: Putar suara saat token ditukar
    playFlippySound();
  };
...
    // BARIS BARU: Putar suara saat token dipilih
    playFlippySound();
  };
...
      // BARIS BARU: Putar suara saat swap berhasil
      playFlippySound();
      setAmountIn('');
      setAmountOut('0');
      setPairPrice('');
